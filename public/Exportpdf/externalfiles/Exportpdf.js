...
class ExportPdfFiles { 
    ...
    async getExperienceUserWisePDF(responseData) {
        ...
        const bootstrapCssFilePath = path.join(__dirname, 'bootstrape', 'css' , 'bootstrap.min.css');
        const bootstrapFontFilePath = path.join(__dirname, 'bootstrape', 'fonts' , 'bootstrap-icons.min.css');
        const bootstrapJsFilePath = path.join(__dirname, 'bootstrape', 'js' , 'bootstrap.bundle.min.js');
        ...
        let fontsContent = fs.readFileSync(bootstrapFontFilePath, 'utf8');
        htmlContent = `<style>${fontsContent}</style>` + htmlContent;
        

        let bootstrapCssContent = fs.readFileSync(bootstrapCssFilePath, 'utf8');
        htmlContent = `<style>${bootstrapCssContent}</style>` + htmlContent;

        let bootstrapJsContent = fs.readFileSync(bootstrapJsFilePath, 'utf8');
        htmlContent = `<script>${bootstrapJsContent}</script>` + htmlContent;
        ...
        return new Promise((resolve, reject) => {
            pdf.create(htmlContent).toFile(path.join(__dirname,'output','output.pdf'), (err, res) => {
                console.log("res",res);
                if (err) {
                    console.log(err);
